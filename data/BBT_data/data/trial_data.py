...
class TrialData(H5Data):
    def __init__(self, subject: str, trial, cfg) -> None:
        ...
        # Path to trigger times / metadata (Brain Treebank layout)
        self.trigger_times_file = os.path.join(
            dataset_dir, "subject_timings", f"{subject}_{trial}_timings.csv"
        )
        self.metadata_file = os.path.join(
            dataset_dir, "subject_metadata", f"{subject}_{trial}_metadata.json"
        )
        self.movie_id, _ = self.get_metadata()

        # Path to transcript csv file
        self.transcript_file = os.path.join(dataset_dir, f'transcripts/{self.movie_id}/features.csv')
