...
class TrialDataReader(H5DataReader):
    ...
    def get_aligned_linguistic_control_matrix(self, duration: int=3, interval_duration=None, onsets_only=True):
        ...
        print(f"DEBUG:  Filtered_data shape: {filtered_data.shape}, ")
        print(f"DEBUG:  Non-word intervals: {len(valid_non_word_intervals)}, Word intervals: {len(word_intervals)}")
        print(f"DEBUG: word intervals shape: {len(word_intervals)}") #print first 5 intervals
        # all_word_samples = np.stack([filtered_data[:, start:end] for (start,end) in word_intervals])
        # # all_word_samples = np.empty((filtered_data.shape[0], len(word_intervals), input_window_duration))
        # # for i, (start, end) in tqdm(enumerate(word_intervals)):
        # #     try:
        # #         all_word_samples[:, i, :] = filtered_data[:, start:end]
        # #     except ValueError as err:
        # #         print(f"Skipping interval {i} due to ValueError: {err}")
        # #         continue
        # # # except ValueError as err:
        # #     print()


        valid_word_intervals = []
        for i, (start, end) in enumerate(word_intervals):
            actual_length = end - start
            if actual_length != input_window_duration:
                print(f"WARNING: Interval {i} has invalid length {actual_length} (expected {input_window_duration})")
                print(f"  Start: {start}, End: {end}, Delta: {end-start}")
                continue
                
            if start < 0 or end > filtered_data.shape[1]:
                print(f"WARNING: Interval {i} out of bounds [{start}, {end}]")
                continue
                
            valid_word_intervals.append((start, end))
        
        print(f"DEBUG: Valid word intervals: {len(valid_word_intervals)}/{len(word_intervals)}")

        # 2. Build arrays with safe stacking
        try:
            all_word_samples = np.stack([filtered_data[:, start:end] for (start, end) in valid_word_intervals])
        except Exception as e:
            # Detailed error analysis
            print(f"CRITICAL: Stacking failed for word samples: {str(e)}")
            lengths = [end - start for (start, end) in valid_word_intervals]
            print(f"  Min length: {min(lengths)}, Max length: {max(lengths)}")
            print(f"  Unique lengths: {set(lengths)}")
            print("First 5 intervals:")
            for i, (s, e) in enumerate(valid_word_intervals[:5]):
                print(f"  {i}: [{s}, {e}] length={e-s}")
            raise
        
        # 3. Validate non-word intervals (already has bounds check)
        print(f"DEBUG: Non-word intervals to process: {len(valid_non_word_intervals)}")
        try:
            all_non_word_samples = np.stack([filtered_data[:, start:end] for (start, end) in valid_non_word_intervals])
        except Exception as e:
            print(f"CRITICAL: Stacking failed for non-word samples: {str(e)}")
            lengths = [end - start for (start, end) in valid_non_word_intervals]
            print(f"  Min length: {min(lengths)}, Max length: {max(lengths)}")
            raise

        # ... [existing balancing code] ...

        # 4. Final shape validation
        print(f"DEBUG: Final word samples shape: {all_word_samples.shape}")
        print(f"DEBUG: Final non-word samples shape: {all_non_word_samples.shape}")
        ...
        print(f"DEBUG: all_word_samples shape: {all_word_samples.shape}, all_non_word_samples shape: {all_non_word_samples.shape}")
