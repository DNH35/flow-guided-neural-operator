...
class H5DataReader:
    ...
    #     print("DEBUG SHAPE: ", np.shape(sel_nbr_data))
    ...
    def laplacian_rereference(self, data_arr):
        ...
        # Create an empty list to store the average of neighboring electrodes for each electrode
        avg_nbr_data = []
        
        # Process each electrode individually
        for i, idx_list in enumerate(sel_nbrs_idxs):
            if len(idx_list) == 0:
                # If an electrode has no neighbors, use its own data (no re-referencing)
                # or use a placeholder value like zeros, depending on your needs
                avg_nbr_data.append(np.zeros_like(data_arr[i]))
                print(f"Warning: Electrode {sel_labels[i]} has no neighbors for re-referencing")
            else:
                # Get data for all neighbors of this electrode
                nbr_data = np.array([adj_data_arr[idx] for idx in idx_list])
                # Filter the data if needed
                if hasattr(self, 'filter_data'):
                    nbr_data = self.filter_data(nbr_data)
                # Calculate the mean across neighbors
                avg_nbr_data.append(np.mean(nbr_data, axis=0))
        
        # Convert to numpy array after ensuring consistent shapes
        avg_nbr_data = np.array(avg_nbr_data)
        
        # Apply Laplacian re-referencing
        laplacian = data_arr - avg_nbr_data
    ...
    def select_electrodes(self, selected) -> np.ndarray:
        ...
        '''
        labels = self.trial_data.get_brain_region_localization()
        print("DEBUG: Selected electrodes: ", selected)
        for e in selected:
            if e not in labels:
                print(f"ERROR: Electrode {e} not in selected electrodes")
            assert e in labels
